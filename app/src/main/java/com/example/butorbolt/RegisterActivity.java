...
public class RegisterActivity extends AppCompatActivity implements AdapterView.OnItemSelectedListener {
    ...
    public void register(View view) {
        ...
        if(passwordEditText.getText().length() < 6){
            Toast.makeText(RegisterActivity.this, "A Jelszónak legalább 6 karakterből kell állnia! ", Toast.LENGTH_LONG).show();
            return;
        }

        if (!android.util.Patterns.EMAIL_ADDRESS.matcher(email).matches()) {
            Toast.makeText(RegisterActivity.this, "Érvénytelen email cím! ", Toast.LENGTH_LONG).show();
            return;
        }
        ...
        mAuth.createUserWithEmailAndPassword(email, password).addOnCompleteListener(this, new OnCompleteListener<AuthResult>() {
            ...
            public void onComplete(@NonNull Task<AuthResult> task) {
                if(task.isSuccessful()) {
                    ...
                    Toast.makeText(RegisterActivity.this, "Regisztráció sikertelen! ", Toast.LENGTH_LONG).show();
                }
            }
        });
    }
